...
export class TransactionComponent implements OnInit, AfterViewChecked {
	...
	loadTransactions() {
		...
		this.transactionService
			.getTransactions(this.listingOption)
			.pipe(
				finalize(() => {
	...
	private disposeCarousel(): void {
		...
		if (this.accountCarousel && this.slidHandler) {
			try {
				this.accountCarousel.nativeElement.removeEventListener(
					'slid.bs.carousel',
					this.slidHandler,
				);
			} catch {
	...
	sortEvent(result) {
		this.listingOption.orderBy =
			result.sortDirection == 'desc' ? result.name + '-' : result.name;
		this.loadTransactions();
	}
	...
	async onEdit(item) {
		this.transactionService
			...
			.subscribe({
				next: async (res) => {
					const result = await this.modalService.open(TransactionUpdateComponent, {
	...
	onDelete(item) {
		this.toastService
			.confirm(ToastType.CONFIRMATION, TransactionConstants.DELETECONFIRMATION)
			.then((it) => {
				if (it) {
					this.isLoading = true;
					this.loadingService.show();
					this.transactionService.remove(item.id).subscribe(() => {
