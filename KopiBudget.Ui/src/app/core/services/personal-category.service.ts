...
export class PersonalCategoryService extends GenericService {
	...
	getPersonalCategories(listingOption): Observable<GenericListingResult<PersonalCategoryDto[]>> {
		const queryParams = this.setQueryParameters(listingOption);

		return this.get<any>(`${this.controller}PersonalCategorys?${queryParams}`, null, true).pipe(
			map((res) => mapItemsToGenericListing<PersonalCategoryDto[]>(res.data)),
		);
	}
	...
	dropdown(dropdownListingOption): Observable<GenericListingResult<PersonalCategoryFragment[]>> {
		...
		return this.get<any>(`${this.controller}Dropdown?${queryParams}`, null, true).pipe(
			map((res) => mapItemsToGenericListing<PersonalCategoryFragment[]>(res.data)),
		);
	}
}
